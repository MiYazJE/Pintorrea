...
async function roomsCtrl() {
    ...
    function guessWord(io, socket, { user, room, guess }) {
        ...
        if (
            ...
            if (game.everyBodyGuessed()) {
                ...
                const finalStatus = 'Todo el mundo ha adivinado la palabra!';
                restartGame(io, room, finalStatus);
            }
        } else {
            io.to(room).emit('message', {
                name: user.name,
    ...
    function initGameLoop(io, roomName) {
        ...
        let interval = setInterval(() => {
            if (room.time == 0) {
                const finalStatus = 'El tiempo ha terminado!';
                restartGame(io, roomName, finalStatus);
