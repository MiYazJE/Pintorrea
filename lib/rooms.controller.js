...
function roomsController(io) {
    ...
    function deletePlayer(user) {
        ...
        currentRoom.players--;
        console.log('players in', user.roomName, currentRoom.players);
        ...
        if (privateRooms.has(user.roomName && currentRoom.players == 0)) {
            clearInterval(currentRoom.gameLoop);
            rooms.delete(user.roomName);
        }
        else if (currentRoom.players === 0) {
            clearInterval(currentRoom.gameLoop);
