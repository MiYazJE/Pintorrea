...
function roomsController(io) {
    ...
    function joinPrivateRoom(user, id) {
        ...
        room.players++;
        rooms.set(id, room);
        io.to(id).emit('updateSettings', { room: mapPrivateRoom(room) });
    }
    ...
    function updateSettingsPrivateRoom(io, room) {
        ...
        io.to(room.id).emit('updateSettings', { room });
    }
    ...
    function handleUserDeletePrivateRoom(room, user) {
        ...
        if (room.players === 0) {
            ...
            console.log('---AFTER---')
            console.log(rooms.get(room.id).allUsers.forEach((a) => console.log(a.name)))
            io.to(room.id).emit('updateSettings', { room: mapPrivateRoom(rooms.get(room.id)) });
