module.exports = async (io) => {
    ...
    io.on('connect', (socket) => {
        ...
        socket.on('guessWord', ({ user, guess, room }) => {
            ...
            else if(notIncludesActualWord(guess, game.getActualWord())) {
        ...
        function startRound(roomName) {
            ...
            // console.log(words)
            io.to(room).emit('gameStatus', { users: room.game.getUsers() });
...
function notIncludesActualWord(guess, actualWord) {
    return !guess.split(' ').includes(actualWord);
}