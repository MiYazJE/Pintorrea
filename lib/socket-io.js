...
module.exports = (io) => {
    ...
    io.on('connect', (socket) => {
        socket.on('joinPrivateRoom', ({ user, id }) => {
            ...
            const socketsRoom = io.sockets.adapter.rooms[id];
            const sockets = [...Object.keys(socketsRoom.sockets)];
            console.log(sockets)
            // let ns = io.of("/");
            // let temp = ns.connected[sockets[0]]
            // console.log(temp.emit);
        });
        ...
        socket.on('disconnect', () => {
            ...
            console.log('gameRoomUser', gameRoomUser)
            if (gameRoomUser) {
            ...
            console.log('privateGameUser', privateGameUser)
            if (privateGameUser) {
